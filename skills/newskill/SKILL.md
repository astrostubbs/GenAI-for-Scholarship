...
Ask the user (via AskUserQuestion): "Should this skill support a `-h` flag that prints usage help? This makes the skill self-documenting, like a Unix command. If yes, we'll also ask about any other command-line flags it should support (e.g., `-v` for verbose, `-o` for output file)."

If the user wants a `-h` flag:
- Ask what other flags/options would be useful for this skill
- The generated skill will include a "Step 0: Parse Arguments" section that checks for `-h` and prints a man-page-style help text, then stops
- All flags will be documented in the help text and in the `argument-hint` frontmatter
...
- A "Step 0: Parse Arguments" section at the top of the instructions that checks for flags
- A help text block printed when `-h` is detected (then stop, do nothing else)
- The help text should follow man-page conventions: Usage line, description, options list, examples
